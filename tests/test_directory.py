...
class DatabaseDirectoryTests(unittest.TestCase):
    ...
    def test_do_not_merge_metadata(self):
        ...
        However, metadata.json is still generated if some data files are not covered.
        """
        with TestFixture(
            Datasets(uncompressed=True, official=True, bulk_metadata=True), flatten=True
        ) as temp_dir:
...
class DatabaseDirectoryInvalidFilesTests(unittest.TestCase):
    ...
    def test_individual_metadata_file_is_corrupt(self):
        """Tests parsing a market metadata file which is not valid JSON."""
    ...
    def test_individual_metadata_file_is_not_a_dict(self):
        """Tests parsing a market metadata file which is not a dict."""
    ...
    def test_individual_metadata_file_does_not_contain_a_market_id_field(self):
        """Tests parsing a market metadata.json file which is missing a market ID field."""
    ...
    def test_market_data_file_is_corrupt(self):
        """Tests parsing corrupt market data files."""
        workdir = self.temp_dir / "corrupt"
        Datasets(corrupt=True, missing_metadata=True).copy_files(workdir, flatten=True)
        data_files = [workdir / "1.221089567.zip", workdir / "1.223716890"]
        dd = DatabaseDirectory(workdir, data_files=data_files)
        ...
        for msg, file in zip(error_messages, data_files):
            self.assertRegex(msg, f"Error parsing.*{file.name}")

    def test_market_data_file_contains_no_metadata(self):
        """Tests parsing a valid market data file without metadata."""
        # workdir = self.temp_dir / "missing_metadata"
        # Datasets(missing_metadata=True).copy_files(workdir, flatten=True)
        file = self.temp_dir / "1.44445555.json"
        file.write_text("{}\n{}\n{}")
        dd = DatabaseDirectory(self.temp_dir, data_files=[file])
        ...
        self.assertEqual(len(error_messages), 1)
        self.assertRegex(
            error_messages[0], f"Market definition missing in.*{file.name}"
        )
